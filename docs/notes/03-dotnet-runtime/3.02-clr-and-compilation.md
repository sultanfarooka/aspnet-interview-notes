...
language, and that is what goes in the `.dll`. Then, while the app runs, the **CLR** turns
that IL into real machine code for the actual processor.
...
- **CTS** and **CLS** are the rules that let C#, VB.NET and F# share the same compiled code.
...
|---|---|---|
| **Roslyn** | *the C# compiler* | The thing that runs when you press Build. Turns `.cs` files into IL. |
| **IL** | *a language* | Intermediate Language. A simple instruction set that is **not tied to any CPU**. Your `.dll` contains this, not machine code. |
| **Assembly** | *a file* | The `.dll` or `.exe` Roslyn produces. Contains IL plus metadata describing every type in it. |
| **CLR** | *the runtime engine* | Common Language Runtime. The program that runs your code. It does JIT compilation, memory allocation, garbage collection and exception handling. |
| **JIT** | *a compilation step* | Just-In-Time. The CLR converting IL into machine code **at the moment a method is first called**, then caching the result. |
| **Tiered compilation** | *a JIT strategy* | Compile quickly first for fast startup, then recompile hot methods with full optimisation in the background. |
...
| **CLS** | *a specification* | Common Language Specification. A **subset** of CTS containing only features every .NET language supports. |
| **AOT** | *a compilation step* | Ahead-Of-Time. Compiling all the way to native machine code at **build** time, with no IL and no JIT left. |
| **ReadyToRun (R2R)** | *a compilation step* | A middle ground. Pre-compiles to machine code but **keeps the IL** as a fallback. |
| **Trimming** | *a build step* | Removing unused code to shrink the output. Used with AOT, and the reason reflection breaks there. |
