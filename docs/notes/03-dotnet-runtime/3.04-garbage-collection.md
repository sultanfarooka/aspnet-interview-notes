...
- It works out what is still in use by **starting from known roots and following every reference**. Anything it cannot reach is garbage.
...
| **Stack** | *a region of memory* | Where local variables live. Cleared automatically when a method returns — the GC is not involved. |
| **Garbage collector (GC)** | *part of the CLR* | The component that finds and frees unreachable objects. |
| **Root** | *a starting point for the search* | Somewhere the GC knows is definitely live: local variables of running methods, `static` fields, CPU registers. |
...
| **Mark / Sweep / Compact** | *the three phases of a collection* | Mark what is reachable → free what is not → slide survivors together to close the gaps. |
...
| **Fragmentation** | *a memory problem* | Free space exists but is split into scattered gaps, so a large request still fails. |
...
In older languages like C, you asked for memory and had to give it back yourself:
...
Two things go wrong constantly. **Forget to free** and the memory is lost forever — a leak.
**Free it twice, or use it after freeing**, and the program corrupts itself or crashes. Whole
categories of security vulnerability come from exactly this.

.NET removes the problem by tracking it for you. You create objects and stop using them. The
**garbage collector** notices and reclaims the memory.
...
The GC does **not** count how many things point at an object. It does something simpler and
more reliable.

It starts from places it knows are definitely live — the **roots**:
...
The hooks are the roots. Anything you can trace back to a hook by following strings is kept.
Anything with no path back to a hook — even if it is tied to other objects — is rubbish and
gets swept away.

That last part matters. Two objects pointing at each other with no path from a root are
...
The two red objects point at each other, but nothing reaches them from a root. Both are
collected.
...
### The simple version — the three phases

When a collection runs, the GC does three things:

1. **Mark.** Start at the roots, follow every reference, mark everything reachable.
2. **Sweep.** Everything unmarked is garbage. Its memory is now free.
3. **Compact.** Slide the survivors together so free space is one continuous block rather than
   scattered gaps.

That third phase has a consequence worth knowing: **objects move**, so an object's address is
not stable. That is fine in normal C#, and it is exactly why you need `fixed` and pinning when
handing a pointer to native code — see [3.3](3.03-managed-vs-unmanaged.md).

Compaction also makes allocation extremely fast. Because free memory is one continuous block,
allocating is just moving a pointer forward. It is close to free.

### How it actually works — generations

Everything is built on one observation:

> **Most objects die very young.** A string built inside a method, a DTO created for one
> request, the result of a LINQ query — the vast majority become garbage almost immediately. A
> small minority live for the whole application.

Scanning every object every time would be wasteful, since most old ones are still alive. So
the GC splits the heap into three **generations** by age:
...
- Survive a collection and you are **promoted** to the next generation.
- A Gen 0 collection looks only at Gen 0. Often under a millisecond.
- A Gen 2 collection examines **everything**, including Gen 0 and Gen 1. This is a **full
  collection** and it is the expensive one.
...
So the design makes the common case — short-lived objects — almost free, and accepts that the
rare case is slow.

**Why this matters for your code:** an object that survives into Gen 2 is expensive to clean up
later. Caching things "just in case" pushes objects into Gen 2 and makes your full collections
slower and more frequent.

### The Large Object Heap

Objects of **85,000 bytes or more** go somewhere separate: the **Large Object Heap**, or LOH.
Usually big arrays, large strings, or buffers.
...
The result is an application whose memory keeps climbing even though nothing is really leaking.
Repeatedly resizing large buffers is a classic cause.

**The fix is not to force compaction** — that exists but is very expensive:
...
Two settings matter in production, and interviewers do ask.
...
| GC threads | One | One per core, in parallel |
| Optimised for | Low latency, low memory | Maximum throughput |
| Default for | Desktop apps | **ASP.NET Core** |

Server GC gives each core its own heap and collection thread, so collections finish much
faster on multi-core hardware. The cost is higher memory use, since there are several heaps.

```xml
<ServerGarbageCollection>true</ServerGarbageCollection>
```

⚠️ **This is a real container gotcha.** Server GC assumes it owns the machine. In a container
limited to one core with a tight memory cap, several heaps waste memory and can push you past
the limit — which the orchestrator handles by killing the container. If you run small
containers, test with Workstation GC:

```xml
<ServerGarbageCollection>false</ServerGarbageCollection>
```

Modern .NET reads container limits much better than it used to, and .NET 9 enabled **DATAS**
by default, which adapts the number of heaps to actual load. Still worth checking if
containers are being killed for memory.

**Concurrent / background GC** lets most of a Gen 2 collection run on a background thread
while your app keeps working, so the pause is much shorter. On by default; rarely changed.
...
- triggers a **full Gen 2 collection**, the most expensive kind
...
dropping something very large. If you are reaching for it to fix a memory problem, the problem
is a reference being held somewhere — see [3.5](3.05-memory-leaks.md).
...
```csharp
// The GC will eventually free this object's memory,
// but the file stays LOCKED until Dispose runs.
await using var stream = File.OpenRead(path);
```

That distinction is the practical takeaway — see
...
// 3. Avoid the allocation entirely when slicing
...
marking everything it can reach. Anything unmarked is unreachable and therefore garbage. An
...
surviving a collection is promoted upward. The design rests on the observation that most
objects die very young — temporary strings, DTOs, LINQ results. So collecting only Gen 0
catches the vast majority of garbage while examining a small fraction of the heap, which makes
it very fast. Gen 2 collections examine everything and are expensive, so the GC does them
rarely. The practical implication is that keeping objects alive longer than necessary pushes
them into Gen 2 and makes your expensive collections more frequent.

**Q: What is the Large Object Heap and why does it cause problems?**

Objects of 85,000 bytes or more — usually large arrays or strings — go on a separate heap. It
is collected only as part of a Gen 2 collection, so large objects survive longer, and it is not
compacted by default because moving very large objects is costly. Not compacting means
fragmentation: after allocating and releasing many differently-sized large objects you can have
plenty of free memory in total but no single contiguous block big enough for the next request,
so the process grows. The symptom is memory climbing steadily without an actual leak. The fix
is to stop churning large objects — pool buffers with `ArrayPool<T>` rather than allocating
fresh ones.

**Q: What is the difference between Workstation and Server GC?**

Workstation GC uses a single heap and a single collection thread, favouring low latency and low
memory. Server GC creates one heap and one collection thread per CPU core, so collections run
in parallel and finish much faster on multi-core hardware, at the cost of higher memory use.
Server GC is the default for ASP.NET Core because throughput matters more there. The thing to
watch is containers — Server GC assumes it owns the machine, so in a small container with a
tight memory limit the multiple heaps can push you over and get the container killed. Modern
.NET reads container limits much better, and .NET 9 turned on DATAS which adapts heap count
dynamically, but it is still worth testing.
...
are benchmarking, where you want a known starting state, and immediately after deliberately
dropping something very large. If someone is calling it to fix a memory problem, the real issue
is something holding a reference, and forcing collections cannot help because the object is
still reachable.
...
- **"The GC handles all cleanup."** It handles **memory only**. Files, sockets and connections
  need `IDisposable`.
- **Calling `GC.Collect()` to fix memory usage.** If an object is still referenced, no amount
  of collecting frees it.
- **Forcing collection promotes objects**, making things worse rather than better.
- **Objects move during compaction.** An object's address is not stable, which is why pinning
  exists for interop.
- **The LOH threshold is 85,000 bytes** — smaller than people expect. An array of about 21,000
  `int`s already qualifies.
