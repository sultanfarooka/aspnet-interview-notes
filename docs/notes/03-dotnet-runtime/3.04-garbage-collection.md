...
- The GC automatically frees objects **nobody is using any more**. You never write "delete this".
- It decides what is in use by **starting from known roots and following every reference**. Anything it cannot reach is garbage.
- It sorts objects into **three generations** based on age, and mostly only cleans the youngest one — because most objects die young.
