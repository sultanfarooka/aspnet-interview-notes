...
| "REST just means JSON over HTTP" | It means resources at URLs acted on by standard verbs. `POST /api/getOrder` is **RPC wearing REST's clothes**. | [10.1](notes/10-web-api-rest/10.01-rest-principles.md) |
| "Our API is fully RESTful" | Without hypermedia it's **Level 2** on the Richardson model — like nearly every API. Say so; the honesty reads better than the claim. | [10.1](notes/10-web-api-rest/10.01-rest-principles.md) |
| "Session state in an API is fine" | It breaks **statelessness** and works perfectly on one server, then fails on scale-out when request two lands elsewhere. | [10.1](notes/10-web-api-rest/10.01-rest-principles.md) |
| "Every safe verb is idempotent and vice versa" | Only the first half. `DELETE` is idempotent and definitely **not safe**. | [10.2](notes/10-web-api-rest/10.02-http-verbs-idempotency.md) |
| "`DELETE` isn't idempotent — the second call 404s" | Idempotency is about **server state**, not the status code. After one call it's gone; after five it's still gone. | [10.2](notes/10-web-api-rest/10.02-http-verbs-idempotency.md) |
| "`PUT` updates the fields I send" | It **replaces the whole resource**. Anything absent from the payload is nulled — the classic hand-rolled `PUT` bug. | [10.2](notes/10-web-api-rest/10.02-http-verbs-idempotency.md) |
| "A `GET` with a small side effect is harmless" | Crawlers, link previews and prefetch will trigger it. `GET /users/5/delete` has wiped real databases. | [10.2](notes/10-web-api-rest/10.02-http-verbs-idempotency.md) |
| "`patch.ApplyTo(dto)` throws on a bad patch" | It records the failure in `ModelState` and carries on. Skip the check and an invalid patch returns 204 having done nothing. | [10.2](notes/10-web-api-rest/10.02-http-verbs-idempotency.md) |
| "401 means you're not allowed" | 401 is **misnamed** — it means *unauthenticated*. 403 means we know who you are and the answer is still no. | [10.3](notes/10-web-api-rest/10.03-status-codes.md) |
| "200 with `{success: false}` is fine, the body says so" | It defeats retries, caching and monitoring. Your dashboards show 100% success while users fail. | [10.3](notes/10-web-api-rest/10.03-status-codes.md) |
| "An empty search result is a 404" | It's **200 with `[]`**. The collection exists and the query succeeded — it just matched nothing. | [10.3](notes/10-web-api-rest/10.03-status-codes.md) |
| "A duplicate email on signup is a 400" | The request was valid; it **conflicts with current state**. That's 409 — and it tells the client to refetch, not to fix their payload. | [10.3](notes/10-web-api-rest/10.03-status-codes.md) |
| "`[ApiController]` is optional sugar" | It makes attribute routing **mandatory** — the app throws at startup without a route attribute. | [10.4](notes/10-web-api-rest/10.04-apicontroller-attribute.md) |
| "A filter object on a GET binds from the query string" | It's a complex type, so it infers **`[FromBody]`** and binds nothing. `[FromQuery]` is required. | [10.4](notes/10-web-api-rest/10.04-apicontroller-attribute.md) |
| "`[ApiController]` validates my model" | It validates **annotations on the bound model**. Objects you build afterwards — a patched DTO, a mapped entity — are never revalidated. | [10.4](notes/10-web-api-rest/10.04-apicontroller-attribute.md) |
| "`Accept: application/xml` gets XML" | Only JSON ships by default, and an unsatisfiable `Accept` silently gets **JSON anyway** unless `ReturnHttpNotAcceptable` is set. | [10.5](notes/10-web-api-rest/10.05-content-negotiation-json.md) |
| "JSON casing matches my C# properties" | Output is **camelCase**, so a JS client reading `response.OrderId` gets `undefined` — silently. | [10.5](notes/10-web-api-rest/10.05-content-negotiation-json.md) |
| "Enum values are stable in the JSON" | They serialize as **numbers**, so inserting a member mid-enum renumbers the contract. Add `JsonStringEnumConverter`. | [10.5](notes/10-web-api-rest/10.05-content-negotiation-json.md) |
| "`AddNewtonsoftJson()` only affects the action that needs it" | It changes serialization **app-wide** — null handling, dates and casing included. | [10.5](notes/10-web-api-rest/10.05-content-negotiation-json.md) |
| "Adding a field is always safe" | Adding is. **Changing a default sort order or page size is breaking in practice**, even though the schema never moved. | [10.6](notes/10-web-api-rest/10.06-api-versioning.md) |
| "We'll retire v1 when nobody uses it" | Without per-version usage logging you can never prove that. **Measure first** — otherwise the answer is always "leave it running". | [10.6](notes/10-web-api-rest/10.06-api-versioning.md) |
| "Deep pagination is just slower" | `OFFSET 100000` reads **100,020 rows and discards 100,000**. Page 1 is instant and page 5000 times out. | [10.7](notes/10-web-api-rest/10.07-pagination-filtering.md) |
| "Offset pagination returns every row eventually" | Not on a table being written to. Rows shift between requests, so you **silently skip some and see others twice**. | [10.7](notes/10-web-api-rest/10.07-pagination-filtering.md) |
| "Sorting by status is fine" | Not unless it's a **total** ordering. Ties order arbitrarily, so rows land on two pages and others vanish. Append the primary key. | [10.7](notes/10-web-api-rest/10.07-pagination-filtering.md) |
| "`?pageSize=` just controls the page size" | Uncapped it's a **DoS vector** — one request, a full table load and serialization. Clamp it server-side. | [10.7](notes/10-web-api-rest/10.07-pagination-filtering.md) |
| "`ProblemDetails` is on by default" | For status codes returned from actions, yes. **Unhandled exceptions and routing 404s are not** — you need `UseExceptionHandler` and `UseStatusCodePages`. | [10.8](notes/10-web-api-rest/10.08-problem-details.md) |
| "Including the exception message helps debugging" | On a 500 it leaks **hostnames, file paths and database account names**. Return a `traceId`; log the detail. | [10.8](notes/10-web-api-rest/10.08-problem-details.md) |
| "My `IExceptionHandler` chain covers everything" | If every handler returns `false` the exception **propagates to a bare 500**. The last one must be an unconditional catch-all. | [10.8](notes/10-web-api-rest/10.08-problem-details.md) |
| "`AddOpenApi()` gives me Swagger UI" | It generates the **document only**. `/swagger` is a 404 in .NET 9 until you add Scalar or Swagger UI yourself. | [10.9](notes/10-web-api-rest/10.09-openapi-swagger.md) |
| "The generated schema documents my API" | Only what it can infer. Without `[ProducesResponseType]` and NRTs enabled, every error is undocumented and every property optional. | [10.9](notes/10-web-api-rest/10.09-openapi-swagger.md) |
| "CORS secures my API" | It's enforced **by the browser**. curl, Postman and other servers ignore it entirely. Authentication secures your API. | [10.10](notes/10-web-api-rest/10.10-cors.md) |
| "CORS protects against CSRF" | The same-origin policy blocks **reading the response**, not sending the request. The side effect already happened. | [10.10](notes/10-web-api-rest/10.10-cors.md) |
| "CORS works for public endpoints but not authenticated ones" | `UseCors` is after `UseAuthorization`. Preflights carry **no credentials**, so they're 401'd before CORS runs. | [10.10](notes/10-web-api-rest/10.10-cors.md) |
| "`SetIsOriginAllowed(_ => true)` is the workaround" | It reflects any origin back — functionally "allow everything with credentials", sidestepping the spec's guard. **A real vulnerability.** | [10.10](notes/10-web-api-rest/10.10-cors.md) |
| "My `X-Total-Count` header works, JS just can't read it" | Custom headers are hidden from browsers without `Access-Control-Expose-Headers` — including `Location`. | [10.10](notes/10-web-api-rest/10.10-cors.md) |
| "A `record` with `init` can't be over-posted" | Model binding uses `init` setters happily. Protection comes from the property **not existing** on the request type. | [10.11](notes/10-web-api-rest/10.11-dtos-and-mapping.md) |
| "`Include` then `Select` is efficient" | Projecting makes `Include` redundant, and it still **loads everything** first. Drop the `Include`. | [10.11](notes/10-web-api-rest/10.11-dtos-and-mapping.md) |
| "AutoMapper is the standard choice" | It fails at **runtime**, not compile time — and since v15 (2025) it needs a **commercial licence**. Mapperly is source-generated and free. | [10.11](notes/10-web-api-rest/10.11-dtos-and-mapping.md) |
| "Minimal APIs validate my model like controllers do" | There is **no automatic validation**. Data annotations are simply not checked — add an endpoint filter. | [10.12](notes/10-web-api-rest/10.12-minimal-apis.md) |
| "`Results.Ok()` and `TypedResults.Ok()` are the same" | The typed one gives you a **testable return type and inferred OpenAPI schema**. Use `TypedResults`. | [10.12](notes/10-web-api-rest/10.12-minimal-apis.md) |
| "I'll generate a fresh idempotency key on each retry" | That defeats the entire mechanism. The key is generated **once per logical operation**, before the first attempt. | [10.13](notes/10-web-api-rest/10.13-idempotency-etags.md) |
| "Storing the key and the response is enough" | Store a **request hash** too — otherwise a key reused with a different body returns the wrong stored response. | [10.13](notes/10-web-api-rest/10.13-idempotency-etags.md) |
| "`If-None-Match` protects my `PUT`" | That's the **caching** header. Concurrency needs `If-Match`, which returns 412 when someone else saved first. | [10.13](notes/10-web-api-rest/10.13-idempotency-etags.md) |
| "gRPC is just a faster REST" | Different model — contract-first, binary, **HTTP/2 only**, and a browser can't call it without a proxy. | [10.14](notes/10-web-api-rest/10.14-rest-vs-grpc-vs-graphql.md) |
| "GraphQL is strictly better than REST" | It kills **HTTP caching**, is **N+1 by construction** without DataLoader, is a **DoS vector** without depth limits, and moves authorization to every field. | [10.14](notes/10-web-api-rest/10.14-rest-vs-grpc-vs-graphql.md) |
| "Renaming a `.proto` field breaks clients" | Names are free to change. **Reusing or renumbering a field number** is what silently corrupts data. | [10.14](notes/10-web-api-rest/10.14-rest-vs-grpc-vs-graphql.md) |
